const _data = [{
  'photo': 'https://raw.githubusercontent.com/hocgin/WeatherForMac/master/screenshots/weather-macOS.png',
  'name': 'Weather For Mac',
  'version': 'Version 0.0.2',
  'icons': [
    {
      'icon': '<i class="fa fa-lg fa-hashtag project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForMac',
      'title': '源码'
    }, {
      'icon': '<i class="fa fa-lg fa-apple project-icon"></i>',
      'href': 'https://hocg.in/404',
      'title': 'Mac Store'
    }, {
      'icon': '<i class="fa fa-lg fa-download project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForMac/releases',
      'title': '下载'
    }, {
      'icon': '<i class="fa fa-lg fa-share-alt project-icon"></i>',
      'href': '#share',
      'title': '分享'
    }, {
      'icon': '<i class="fa fa-lg fa-pencil project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForMac/issues',
      'title': 'Q&A'
    }, {
      'icon': '<i class="fa fa-lg fa-terminal project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForMac',
      'title': '更新日志'
    }, {
      'icon': '<i class="fa fa-lg fa-question-circle project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForMac',
      'title': 'Help'
    }, {
      'icon': '<i class="fa fa-lg fa-gift project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForMac',
      'title': '其他'
    }
  ]
}, {
  'photo': 'https://raw.githubusercontent.com/hocgin/WeatherForChrome/master/screenshots/weather-chrome.png',
  'name': 'Weather For Chrome',
  'version': 'Version 0.0.2',
  'icons': [
    {
      'icon': '<i class="fa fa-lg fa-hashtag project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForChrome',
      'title': '源码'
    }, {
      'icon': '<i class="fa fa-lg fa-chrome project-icon"></i>',
      'href': 'https://chrome.google.com/webstore/detail/weather/ibieofighcnndjcjchdahdiacjpmkhgf',
      'title': 'Chrome Store'
    }, {
      'icon': '<i class="fa fa-lg fa-download project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForChrome/releases',
      'title': '下载'
    }, {
      'icon': '<i class="fa fa-lg fa-share-alt action-clipboard project-icon" data-clipboard-text="https://github.com/hocgin/WeatherForChrome"></i>',
      'href': '#share',
      'title': '分享'
    }, {
      'icon': '<i class="fa fa-lg fa-pencil project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForChrome/issues',
      'title': 'Q&A'
    }, {
      'icon': '<i class="fa fa-lg fa-terminal project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForChrome',
      'title': '更新日志'
    }, {
      'icon': '<i class="fa fa-lg fa-question-circle project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForChrome',
      'title': 'Help'
    }, {
      'icon': '<i class="fa fa-lg fa-gift project-icon"></i>',
      'href': 'https://github.com/hocgin/WeatherForChrome',
      'title': '其他'
    }
  ]
}]

export default {
  get () {
    return _data
  }
}
